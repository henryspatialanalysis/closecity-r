...
- **Census block.** The smallest unit the Census Bureau publishes. Each one has a
...
- **Isochrone.** The area you can reach from a point within a time limit, drawn as
  a polygon.
- **Catchment.** The reverse of an isochrone: every block that can reach a given
  place.
- **Token.** One unit of API usage. Free routes cost nothing; data routes draw
  down a monthly allowance.

## Reading many pages

List routes return one page at a time. Use `$records()` to follow the pages to the
end and return everything at once.
...
Feature methods return sf by default. Block routes join census-block boundaries
for you (this needs the `tigris` package, which downloads the boundaries once and
caches them). To work with the raw data instead, set `spatial = FALSE`:
