...
Two free calls save you from memorising codes. Use `$destination_types()` to find
the numeric id for a category, and `$places()` to turn a city name into a GEOID and
a centre point.
...
List routes return one page at a time. `$records()` follows the pages to the end.
...
Set `spatial = FALSE` to work with the raw data. Block routes join census-block
boundaries for you, using the `tigris` package to download them once.
