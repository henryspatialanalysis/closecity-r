...
one sf table with one row per (block, category); block boundaries come from
`tigris`, downloaded once and cached. (To search an arbitrary area instead, use
`$blocks_query()` with a centre and radius or a polygon — we do that with a radius
in the other tutorials only to keep their token cost low; a place GEOID pulls the
whole city.)
...
blocks that pass all three. `intersect()` takes two sets, so fold it over the list.
