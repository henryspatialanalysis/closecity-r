...
times for every block in the city. `$records()` reads every page and returns one sf
table with one row per (block, category); block boundaries come from `tigris`,
downloaded once and cached. To search an arbitrary area instead, use
`$blocks_query()` with a centre and radius or a polygon.
